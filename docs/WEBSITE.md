...
editing copy), and the components under `site/.vitepress/theme/`. The words
themselves come from `design/README.md` § Words, shared with the README.
Images and fonts in `site/public/` are generated by `design/build.mjs`,
`design/materials.mjs` and `design/site.mjs` — don't hand-edit them. `site/public/install.sh` is the
